...
due dates, and an AI prompt bar that replans any week (or standing rules
spanning many weeks) on request.
...
- **Storage**: all weeks, tasks, standing rules and settings are saved to
  the browser's `localStorage`. Nothing leaves your machine except the AI
  planning requests below.
- **AI planning**: calls the Google Gemini API directly from the browser
  using your own API key — no backend, no proxy.

## Setup

1. Open `index.html` in a browser. Because it makes cross-origin requests
   to `generativelanguage.googleapis.com`, some browsers block that from a
   plain `file://` page — if requests fail immediately, serve the folder
   locally instead, e.g.:
   ```
   npx serve .
   ```
...
2. Click the ⚙ button in the top bar and paste in a Gemini API key (free
   tier works — get one at aistudio.google.com/apikey). Pick a model —
   Gemini Flash is the default and recommended balance of quality and
   speed; Flash-Lite is faster/cheaper. (Gemini Pro isn't offered here: on
   the free tier it returns a quota error.)
...
sent directly from your browser to Google on every AI planning request —
...
- `index.html` — the entire app (HTML/CSS/JS, no build step, no dependencies
  beyond Google Fonts and the Gemini API).
